...
<div id="main" class="container">
    <div class="p-4 lg:p-8" style="width:45%">
        <div class="text-left">
            <div>
                <h2 class="font-semibold text-xl">TRANSFER STUDENT FROM THE ONE COURSE TO ANOTHER</h2>
                <p>Enter student ID and course from/to in the form below</p>
                <br>
                ...
                >
                    @csrf
                    <p class="text-left pb-5">Transfer student to another course</p>
                    ...
                    <div class="flex flex-grow-1 items-center justify-between">
                        <label for="courseIdFrom">Select a Course name From</label>
                        <?php
                        $coursesList = DB::table('courses')->orderBy('course_name')->get();
                        ?>
                        <select id="courseIdFrom" name="courseIdFrom" class="px-2 py-1 border border-info border-solid">
                    ...
                    <div class="flex flex-grow-1 items-center justify-between">
                        <label for="courseIdTo">Select a Course name To</label>
                        <select id="courseIdTo" name="courseIdTo" class="px-2 py-1 border border-info border-solid">
                            @foreach($coursesList as $course)
                                <option value={{$course->id}}>{{$course->course_name}}</option>
                            @endforeach
                        </select>
